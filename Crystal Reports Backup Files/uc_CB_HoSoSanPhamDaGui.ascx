<%@ Control AutoEventWireup="true" CodeFile="uc_CB_HoSoSanPhamDaGui.ascx.cs" Inherits="UserControls_uc_CB_HoSoSanPhamDaGui"
    Language="C#" %>
<div>
    <span style="font-family: Arial"><strong>CÔNG BỐ &gt;&gt; </strong>
        <asp:HyperLink ID="linkCum" runat="server" /><strong> &gt;&gt; DANH SÁCH SẢN PHẨM ĐÃ
            GỬI </strong></span>
</div>
                ...
                <asp:GridView ID="gvSanPham" runat="server" AllowPaging="True" AutoGenerateColumns="False"
                    ...
                    <Columns>
                        ...
                        <asp:TemplateField HeaderText="K&#253; hiệu">
                            <ItemTemplate>
                                <a href="../ReportForm/HienBaoCao.aspx?HoSoID=<%# Request["HoSoID"].ToString() %>&SanPhamId=<%# Server.UrlEncode(Eval("ID").ToString()) %>&LoaiBaoCao=CBPhieuDanhGia&Format=Word">
                                    <%# Eval("KyHieu")%>
                                </a>
                            </ItemTemplate>
        ...
        <td style="width: 100%; text-align: left">
            <fieldset style="width: 98%">
                ...
                <asp:GridView ID="gvPhi" runat="server" AllowPaging="True" AutoGenerateColumns="False"
                    EmptyDataText="Không có dữ liệu !" PageSize="20" Width="100%" DataKeyNames="ID,TrangThaiID,SoGiayThongBaoLePhi">
                    <Columns>
                        <asp:TemplateField HeaderText="Số th&#244;ng b&#225;o lệ ph&#237;">
                            <ItemStyle Font-Underline="False" />
                            <ItemTemplate>
                                <a href="#" onclick="if(<%# Eval("TrangThaiID") %> == 1) return popCenter('CB_ThongBaoPhi_TaoMoi.aspx?action=edit&HoSoID=<%# Request["HoSoID"].ToString() %>&ThongBaoLePhiID=<%# Eval("ID")%>','CN_ThongBaoPhi_TaoMoi',800,600);                                             
                                             else if (<%# Eval("TrangThaiID") %> != 1) return popCenter('CB_ThongBaoPhi_TaoMoi.aspx?action=view&HoSoID=<%# Request["HoSoID"].ToString() %>&ThongBaoLePhiID=<%# Eval("ID")%>','CN_ThongBaoPhi_TaoMoi',800,600);
