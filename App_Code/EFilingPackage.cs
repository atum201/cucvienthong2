...
namespace AppSrvLib
{
    ...
    public class EFilingPackage
    {
        ...
        public byte[] mPackageHeaderNoMessage = new byte[TOTAL_HEADER_SIZE];
        public byte[] mPackageDataMessage;
        ...
        public void SetTransactionID(string strTransactionID)
        {
            ...
            Array.Copy(ASCIIEncoding.ASCII.GetBytes("a968b921-ed04-443a-8593-804yh64cf6" + strTransactionID), FIRST_INDEX,
        ...
        public void SetFileName(string strTransactionID)
        {
            SetCode(strTransactionID);
        }
        /// <summary>
        /// Get transactionID
        /// </summary>     
        ...
        public string GetStringUserID()
        {
            ...
            byte[] byteUserID = new byte[USER_ID];
            Array.Copy(mPackageHeaderNoMessage, TRANSACTION_TYPE + TRANSACTION_ID,
                                                byteUserID, FIRST_INDEX
                                                , USER_ID);
           
            return EFilingCommunication.RemoveNullCharacter(ASCIIEncoding.ASCII.GetString(byteUserID));
        }
        /// <summary>
        /// Set code 
        /// </summary>
        ...
        /// </summary>
        /// <returns></returns>
        public string GetIntDivisionSize()
        {
            byte[] byteDivisionSize = new byte[DIVISION_SIZE];
            Array.Copy(mPackageHeaderNoMessage, FIRST_INDEX,
                       byteDivisionSize, TRANSACTION_TYPE + TRANSACTION_ID + USER_ID + CODE + TOTAL_BYTES + DIVISION_MAX
        ...
        /// </summary>
        /// <returns>String</returns>
        public string GetIntDivisionOffset()
        {
            byte[] byteDivisionOffset = new byte[DIVISION_OFFSET];
            Array.Copy(mPackageDataMessage, FIRST_INDEX,
                       byteDivisionOffset, TRANSACTION_TYPE + TRANSACTION_ID + USER_ID + CODE + TOTAL_BYTES + DIVISION_MAX + DIVISION_SIZE
        ...
        public byte[] GetbyteApplicationReposeCode()
        {
            ...
            Array.Copy(mPackageHeaderNoMessage, TOTAL_HEADER_SIZE - APPLICATION_REPONSE_CODE,
                    byteApplicationReposeCode, FIRST_INDEX
                                             , APPLICATION_REPONSE_CODE);
        ...
        {
            byte[] byteApplicationReposeCode = new byte[APPLICATION_REPONSE_CODE];
            Array.Copy(mPackageHeaderNoMessage, TOTAL_HEADER_SIZE - APPLICATION_REPONSE_CODE - DIVISION_HASH - DIVISION_RC,
                       byteApplicationReposeCode, FIRST_INDEX
                                                , APPLICATION_REPONSE_CODE);
        ...
        {
            //Tổng số byte kiểu long được Convert sạng Byte16-> Coppy vào mHeader.
            Array.Copy(byteDivisionHash, FIRST_INDEX,
                                        mPackageHeaderNoMessage, TRANSACTION_TYPE + TRANSACTION_ID + USER_ID + CODE + TOTAL_BYTES + DIVISION_MAX + DIVISION_SIZE + DIVISION_OFFSET + APPLICATION_REPONSE_CODE
                                        , DIVISION_HASH);
        }
        /// <summary>
        /// Get DivisionHash
        ...
        ///	TrườngTV        20/06/2008	     Thêm mới        
        ///	</Modified>        
        public void SetDivisionRC(byte[] byteDivisionRC)
        {
            //Tổng số byte kiểu long được Convert sạng Byte16-> Coppy vào mHeader.
            Array.Copy(byteDivisionRC, FIRST_INDEX,
        ...
        {
            //get transaction type
            //byte[] byteDataMesage = new byte[TOTAL_DATA_SIZE];
            //Array.Copy(mPackageDataMessage, FIRST_INDEX, byteDataMesage, TOTAL_HEADER_SIZE, TOTAL_DATA_SIZE);
