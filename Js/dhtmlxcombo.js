...
function dhtmlXComboFromSelect(parent,size){
   ...
   var w= new dhtmlXCombo(z,parent.name,size,s_type,parent.tabIndex,parent);
...
*     @topic: 0
*/
function dhtmlXCombo(parent,name,width,optionType,tabIndex,selectControl){
      if (typeof(parent)=="string")
         parent=document.getElementById(parent);
...
*/
      ...
      {  
         ...
         this.DOMelem_input.onkeyup = this._onKeyUp;
      ...
      dhtmlXCombo.prototype.selectNext = function(step){
         ...
         while (this.optionsArr[z]){
            if (!this.optionsArr[z].isHidden())
               ...
               if (this.DOMlist.style.display=="none"){
                    return this.selectOption(z,false,false);
               }            
            z+=step;
         }
...
*/
      dhtmlXCombo.prototype._onKeyF = function(e){
         ...
         if (ev.keyCode=="13" || ev.keyCode=="9" ){
            //giangum
            var selectIdx=that.getSelectedIndex();
            if (ev.keyCode=="13")
            {
                 if (selectIdx>=0)
                    that.selectOption(selectIdx,false,true);
            }else{
                    that.selectOption(selectIdx>=0?selectIdx:0,false,true);
            } 
...
*     @topic: 0
*/
      dhtmlXCombo.prototype._onKeyUp = function(e){
         var ev=(e||event).keyCode;
         if (ev=="40"||ev=="38"){  //down+up
           return false;
         }
         var that=this.parentNode.combo;
         (e||event).cancelBubble=true;
...
*/
      dhtmlXCombo.prototype._onBlur = function()
      {
          ...
          //giangum
          var selectIdx=self.getSelectedIndex();
          if (selectIdx<0)
             self.selectOption(0,true,true); 
...
*/
      ...
      dhtmlXCombo.prototype._confirmSelection = function(data,status){
        ...
        //kiem tra select option raise event onchange 
        selIndex=this.initSelect.selectedIndex;
        if (this.initSelect.tagName=="SELECT"){            
            for(i=0;i<this.initSelect.length;i++){                          
                    if (this.initSelect.options[i].value==data)      
                        if (selIndex!=i) 
                        {                           
                            this.initSelect.options[i].selected=true;
                            this.initSelect.selectedIndex=i;   
...
*/
      dhtmlXCombo.prototype._selectOption = function(e)
      {
         ...
         var i=0;
         //giangum         
         for (i=0; i<that.DOMlist.childNodes.length; i++) {
            if (that.DOMlist.childNodes[i]==node) break;
         }         
...
*/
   dhtmlXCombo.prototype._toggleSelect = function(e)
   {
      var that=this.combo;
      //giangum
      for (i=0; i<that.optionsArr.length; i++) {
          that.optionsArr[i].hide(false);
      } 
      ...
      ///giangum
      ///calculate to select div
      if (that.DOMlist.childNodes.length>0){
        var topSpace=0;
        var itemHeight=that.DOMlist.childNodes[0].clientHeight;
        var selectedIndex=that.getSelectedIndex();
        topSpace = itemHeight *  selectedIndex -(that.DOMlist.clientHeight/2 - 5);        
        that.DOMlist.scrollTop=topSpace;
      }
...
*/
    dhtmlXCombo.prototype.filterSelf = function(mode)
   {
      ...
      var opt=this.getOptionByLabel(text);   
...
/**
*     @desc: set hidden iframe for IEu
*     @type: private
*     @topic: 2
*/
